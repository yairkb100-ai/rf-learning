...
- 🔑 איפוס סיסמה לתלמידים ומנהלים
- 📊 דשבורד ניהולי עם תמונת מצב כיתתית
...
- 🐘 PostgreSQL 14+ (מ-https://www.postgresql.org/)
...
הדשבורד נותן למנהל תמונת מצב מרוכזת של הכיתה במסך אחד.

**כניסה:** בפאנל הניהול לחץ על **"📊 דשבורד"** (או גש ישירות ל-`/admin/dashboard`).

**מה מוצג:**
- מספר התלמידים והמנהלים במערכת
- מספר הקורסים והמגמות
- ממוצע השלמת פרקים לתלמיד
- ציון ממוצע בבחינות
- **מבחנים הממתינים לבדיקה ידנית** (מודגש אם יש כאלה)
- טבלת חמשת הקורסים עם הציון הממוצע הנמוך ביותר (לזיהוי חומר קשה)

הנתונים מחושבים בזמן אמת מתוך בסיס הנתונים — אין צורך בעדכון ידני.

#### 🔑 איפוס סיסמה למשתמש

כאשר תלמיד או מנהל שכח את סיסמתו, המנהל יכול לאפס אותה.

**שלבים:**
1. בפאנל הניהול לחץ על **"ניהול משתמשים"**
2. מצא את המשתמש ברשימה (תלמידים או מנהלים)
3. לחץ על **"🔑 אפס סיסמה"** ליד שמו
4. הזן סיסמה חדשה — חייבת לכלול אות גדולה, אות קטנה וספרה, באורך 6–8 תווים
5. הסיסמה נכנסת לתוקף **מיד**, והמשתמש יכול להתחבר איתה

> הערה: איפוס הסיסמה פשוט ומיידי — המשתמש אינו נדרש להחליף אותה שוב בכניסה הבאה.
...
# Setup Database
# צור DB ריק בשם: rf_learning
# אין צורך להריץ SQL ידנית — המיגרציות רצות אוטומטית בעליית השרת
```

> 💡 להתקנה מלאה על שרת production ראה את הקובץ `DEPLOY.md` בשורש הפרויקט.
...
# Migrations (ידני — בדרך כלל רץ אוטומטית)
npm run migrate          # הרצת מיגרציות שטרם הורצו
```

### 🔄 מנגנון מיגרציות אוטומטי

כדי למנוע חוסר-התאמה בין הקוד לבין סכמת בסיס הנתונים, המערכת מריצה את
המיגרציות **אוטומטית בכל עליית שרת**:

- הקובץ `backend/src/db/migrate.ts` סורק את `database/*.sql` לפי סדר אלפביתי.
- טבלת מעקב `schema_migrations` רושמת אילו קבצים כבר הורצו, כך שכל קובץ רץ **פעם אחת בלבד**.
- **מנגנון baseline:** על בסיס נתונים קיים (שהוקם ידנית בעבר), הקבצים הקיימים מסומנים כ"הורצו" בלי להריץ אותם מחדש — כדי למנוע כפילות.
- אם מיגרציה נכשלת — השרת **מסרב לעלות**, כדי שלא יעבוד על סכמה שבורה.

**הוספת מיגרציה חדשה:** צור קובץ חדש ב-`database/` בשם ממוין (למשל `migration_stage7_xxx.sql`), והוא ירוץ אוטומטית בעלייה הבאה.

### 👤 מנהל ראשוני (Bootstrap Admin)

על שרת נקי טבלת המשתמשים ריקה. המיגרציה `migration_stage6_bootstrap_admin.sql`
יוצרת אוטומטית **מנהל זמני** — אך ורק אם אין עדיין אף מנהל במערכת:

```
שם משתמש: OA000000000
סיסמה:    Admin123   (זמנית — חובה להחליף!)
```

**הזרימה הנכונה:** מתחברים עם המנהל הזמני → יוצרים מנהל OA אמיתי → **מוחקים את המנהל הזמני**. ראה פירוט מלא ב-`DEPLOY.md`.
...
│   └── package.json
│
├── database/               # SQL schema + migrations (רצות אוטומטית)
│
├── frontend/
│   ├── src/
...
**תשובה:** Node.js 18+ (בדוק עם `node --version`)
...
2. Create → Database
3. שם: `rf_learning`
4. הפעל את השרת — המיגרציות ייצרו את הטבלאות אוטומטית
...
**תשובה:** דרך פאנל הניהול → "ניהול משתמשים" → הוספת משתמש. מנהל יכול גם לאפס סיסמאות משם.

### ❓ "השרת לא עולה ומציג שחסר משתנה סביבה"
**תשובה:** זו התנהגות מכוונת. הגדר את המשתנים החסרים בקובץ `backend/.env` (ראה תבנית ב-`.env.example`) והפעל שוב.

### ❓ "איך מוציאים לProduction?"
**תשובה:** ראה מדריך מלא בקובץ `DEPLOY.md`. בקצרה צריך:
1. Server עם Node.js 18+ ו-PostgreSQL
2. הגדרת קובץ `.env` (כולל סודות JWT ו-`CORS_ORIGIN`)
3. `npm run build` ואז `npm start` (המיגרציות רצות אוטומטית)
4. הגשת `frontend/dist` דרך שרת ווב (Nginx) + SSL
5. **החלפת המנהל הזמני** (ראה `DEPLOY.md`)
